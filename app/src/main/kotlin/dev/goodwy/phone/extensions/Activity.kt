package dev.goodwy.phone.extensions
...
import dev.goodwy.phone.activities.DialerActivity
...
import dev.goodwy.phone.dialogs.SelectSimButtonDialog
import dev.goodwy.phone.helpers.SIM_DIALOG_STYLE_LIST
...
fun Activity.launchSendSMSIntentRecommendation(recipient: String) {
    ...
    if ((0..config.appRecommendationDialogCount).random() == 2 &&
        ...
        NewAppDialog(
            ...
            text = getString(R.string.right_sms_messenger),
...
fun Activity.newAppRecommendation() {
    if (!isNewApp()) {
        if ((0..config.newAppRecommendationDialogCount).random() == 2) {
            val packageName = "relaid.ywdoog.ved".reversed()
            NewAppDialog(
                activity = this,
