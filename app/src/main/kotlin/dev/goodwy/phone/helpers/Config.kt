...
import dev.goodwy.phone.extensions.getPhoneAccountHandleModel
import dev.goodwy.phone.extensions.putPhoneAccountHandle
...
class Config(context: Context) : BaseConfig(context) {
    ...
    var callButtonStyle: Int
        get() = prefs.getInt(CALL_BUTTON_STYLE, IOS17)
    ...
    var quickAnswers: ArrayList<String>
        get(): ArrayList<String> {
            val defaultList = arrayListOf(
                ContextCompat.getString(context, dev.goodwy.phone.R.string.message_call_later),
                ContextCompat.getString(context, dev.goodwy.phone.R.string.message_on_my_way),
                ContextCompat.getString(context, dev.goodwy.phone.R.string.message_cant_talk_right_now)
            )
    ...
    var dialpadHashtagLongClick: Int
        get() = prefs.getInt(DIALPAD_HASHTAG_LONG_CLICK, DIALPAD_LONG_CLICK_SETTINGS_DIALPAD)
    ...
    var filterRecentCalls: Int
        get() = prefs.getInt(FILTER_RECENT_CALLS, FILTER_RECENT_CALLS_ALL)
        set(filterRecentCalls) = prefs.edit { putInt(FILTER_RECENT_CALLS, filterRecentCalls) }

    var dialpadButtonsColor: Int
        get() = prefs.getInt(DIALPAD_BUTTONS_COLOR, ContextCompat.getColor(context, R.color.default_primary_color))
        set(dialpadButtonColor) = prefs.edit { putInt(DIALPAD_BUTTONS_COLOR, dialpadButtonColor) }

    var dialpadButtonsColorStyle: Int
        get() = prefs.getInt(DIALPAD_BUTTONS_STYLE, DIALPAD_BUTTONS_STYLE_DEFAULT)
        set(dialpadButtonsColorStyle) = prefs.edit { putInt(DIALPAD_BUTTONS_STYLE, dialpadButtonsColorStyle) }
